<template>
  ...
  >
      ...
      <div class="flex flex-wrap items-center md:justify-center w-full">
        <AisSortBy
          :items="[
            ...
            {
              value: 'squads_active_members_desc',
              label: translations.members + ' ↙',
            },
            {
              value: 'squads_active_members_asc',
              label: translations.members + ' ↗',
            },
          ...
          :class-names="{
            'ais-SortBy':
              'mt-2 md:mt-0 order-3 md:order-1 w-5/6 md:w-1/2 md:mr-1 w-full  md:w-fit min-w-max',
            'ais-SortBy-select':
              'w-full pl-3 pr-8 py-1 text-sm leading-8 text-gray-700 transition-colors duration-200 ease-in-out bg-white border border-gray-300 rounded appearance-none focus:outline-none focus:ring-2 focus:ring-secondary-200 focus:border-secondary-500',
            'ais-SortBy-option': '',
          }"
        />
        <AisSearchBox
          ...
          :class-names="{
            ...
            'ais-SearchBox-input':
              'w-full px-3 py-1 text-base leading-8 text-gray-700 transition-colors duration-200 ease-in-out bg-white border border-gray-300 rounded outline-none focus:border-secondary-500 focus:ring-2 focus:ring-secondary-200',
            'ais-SearchBox-loadingIndicator': 'absolute right-8 h-full',
            'ais-SearchBox-loadingIcon':
              'w-4 h-full fill-current stroke-current',
    ...
    />
    <div
      class="fixed top-24 bottom-0.5 w-5/6 sm:w-96 shadow-xl left-0 z-30 px-6 py-4 bg-gradient-to-bl rounded-r-md transition-all border-2"
      :class="{
        'translate-x-0 duration-500 to-secondary-200 from-primary-200 border-primary-500':
          sidebarVisible,
        '-translate-x-full ml-8 h-0 duration-300 to-primary-200 from-secondary-200 border-secondary-500':
          !sidebarVisible,
      }"
    >
      <div class="h-full pb-6" @click.self="sidebarVisible = true">
        <div
          class="absolute top-0 right-0 h-full w-8"
          :class="{
            '': sidebarVisible,
        ...
        >
          <AdjustmentsIcon
            class="w-full mt-1 px-1 transition-none cursor-pointer"
            :class="{
        ...
        <div
          class="relative py-4 mx-2 h-full overflow-y-auto space-y-3 transition-transform duration-500"
          v-show="sidebarVisible"
        >
          <AisPanel>
            <template v-slot:header>
              <p
                class="font-medium leading-tight text-base mt-0 mb-2 text-secondary-500"
              >
                Rank
              </p>
            </template>
              ...
              <template v-slot="{ currentRefinement, range, refine }">
                <div class="w-3/4 px-4 mt-4 mb-8">
                  <vue-slider
                    ...
                    :step-style="{
                      boxShadow: '0 0 0 2px rgb(192, 132, 252)',
            ...
            >
              <template v-slot="{ value, refine }">
                ...
                >
                  ...
                  >
                  <span
                    class="inline-block px-2 leading-3 mt-1 py-1 ml-2 text-xs font-semibold uppercase rounded-full text-secondary-600 bg-secondary-200"
          ...
          <AisPanel>
            ...
            <AisRefinementList
              ...
              :searchable-placeholder="
                indexTranslations.filterSearchCountryPlaceholder
              "
              :class-names="{
                'ais-SearchBox-form': 'pr-3 sm:pr-auto',
                'ais-SearchBox-input':
                  'w-full sm:w-3/4 px-3 py-1 text-sm leading-8 text-gray-700 transition-colors duration-200 ease-in-out bg-white border border-gray-300 rounded outline-none focus:border-secondary-500 focus:ring-2 focus:ring-secondary-200',
                ...
                'ais-RefinementList-count':
                  'inline-block px-2 leading-3 mt-1 py-1 ml-2 text-xs font-semibold uppercase rounded-full text-secondary-600 bg-secondary-200',
                'ais-RefinementList-checkbox':
                  'w-3 h-3 focus:outline-none -mt-1 focus:ring-0 opacity-100 text-secondary-500 rounded-full',
              }"
            >
              <template v-slot:noResults="{ query }">
                <span class="text-xs text-gray-500 ml-2"
                  >{{ indexTranslations.noResults }} <q>{{ query }}</q></span
                >
              </template>
            </AisRefinementList>
          </AisPanel>
        ...
        <AisClearRefinements
          class="absolute bottom-2 right-4 sm:right-8"
          v-show="sidebarVisible"
        >
          <template v-slot="{ canRefine, refine }">
            <button
              class="w-full flex items-center px-3 mb-1 text-xs font-bold uppercase lg:text-secondary-500 lg:hover:text-secondary-600 disabled:text-secondary-400 disabled:hover:text-secondary-400 text-secondary-500"
              @click.prevent="
                refine();
                activeMembers = [1, 30];
              "
    ...
    <div>
      ...
      >
        <template v-slot:item="{ item }">
          <Card v-bind="item" :translations="translations">
            <template #name>
    ...
    <div
      class="mt-12 space-y-2 sm:space-y-auto px-6 flex flex-col sm:flex-row text-xs text-gray-500 justify-center items-center sm:justify-between"
    >
      <AisStats>
        <template v-slot="{ nbHits, processingTimeMS }">
          <span class="font-medium">{{ nbHits }}</span>
          {{ indexTranslations.stats }}
...
export default {
  ...
  data() {
    return {
      searchClient: algoliasearch(this.algoliaAppId, this.algoliaClient),
      routing: {
