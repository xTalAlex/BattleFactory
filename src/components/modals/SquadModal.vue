  ...
  >
    <div class="relative flex flex-col p-6">
      ...
      <div class="flex flex-row items-center justify-start w-full h-8 mb-2">
        <h2 class="text-lg font-semibold uppercase" v-text="code"></h2>
      ...
      <p
        class="px-4 py-2 my-4 text-sm font-medium text-gray-500 rounded bg-secondary-50"
        v-text="description"
        v-show="description"
      ></p>
      ...
      <!-- Mini Cards -->
      <div
        class="flex flex-row items-center justify-center mt-2 space-x-5 text-gray-900 cursor-default"
      >
        <!-- Active Members -->
        <div
          class="flex flex-col items-center justify-center w-40 h-20 transition-colors duration-100 ease-in-out border rounded-md border-secondary-200 hover:border-secondary-400"
        >
          <div class="flex flex-row items-center justify-center">
            <UsersGroupIcon margins="mr-3" />

            <span class="font-bold" v-text="active_members"></span>
          </div>
        ...
        <!-- Ranks -->
        <div
          class="flex flex-col items-center justify-center w-40 py-4 transition-colors duration-100 ease-in-out border rounded-md border-secondary-200 hover:border-secondary-400"
        >
          <div class="flex flex-row items-center justify-center">
            <RankIcon :rank="rank" sizes="h-12" />
          </div>
    ...
    <template #confirmButton>
      <button
        type="button"
        class="px-6 py-3 mb-1 mr-1 text-sm font-bold text-white uppercase transition-all duration-150 ease-linear rounded shadow outline-none bg-primary-500 active:bg-primary-600 hover:shadow-lg focus:outline-none"
        @click="visible = false"
      >
        Ok
      </button>
  ...
  computed: {
    ...
    rankLabel() {
      return this.rank >= 1200
        ? "Master " + this.rank
        : this.rank[0].toUpperCase() + this.rank.substring(1);
    },
  },
  ...
  mounted() {
    document.openSquadModal = function openSquadModal(params) {
      document.dispatchEvent(
        new CustomEvent("openSquadModal", { detail: params })
      );
    };
